...
void print_result(int n, int* suma) {
	ofstream out("C:/Facultate/Cursuri facultate sem 5/PPD/Labs/Teme/lab3/Lab3-PPD-v3/lab3-PPD-v3/lab3-PPD-v3/Numar3.txt");
	for (int i = n - 1; i >= 0; i--)
		if (i == n - 1) {
...
int main() {
	...
	ifstream in1("C:/Facultate/Cursuri facultate sem 5/PPD/Labs/Teme/lab3/Lab3-PPD-v3/lab3-PPD-v3/lab3-PPD-v3/Numar1.txt");
	ifstream in2("C:/Facultate/Cursuri facultate sem 5/PPD/Labs/Teme/lab3/Lab3-PPD-v3/lab3-PPD-v3/lab3-PPD-v3/Numar2.txt");
	in1 >> n1;
	in2 >> n2;
	...
	// Varianta 1
		...
		if (rank == 0) {
			...
			for (int i = n - 1; i >= 0; i--) {
				...
				if (cifre_citite == cifre_per_proces[id_proces_curent]) {
					...
					MPI_Isend(numar1 + i, cifre_per_proces[id_proces_curent], MPI_INT, id_proces_curent, 20, MPI_COMM_WORLD, &req1);
					MPI_Wait(&req1, MPI_STATUSES_IGNORE);
					MPI_Isend(numar2 + i, cifre_per_proces[id_proces_curent], MPI_INT, id_proces_curent, 30, MPI_COMM_WORLD, &req2);
			...
			vector<MPI_Request> resultReq;
			resultReq.resize(nr_procese + 1);
			...
			for (int i = 1; i < nr_procese; i++)
			{
				...
				MPI_Irecv(suma + poz, cifre_per_proces[i], MPI_INT, i, 40, MPI_COMM_WORLD, &resultReq[i]);
			}
		...
		//proces != 0 
			...
			MPI_Status out;
			MPI_Request req1, req2;
			MPI_Irecv(subnumar1, cat, MPI_INT, 0, 20, MPI_COMM_WORLD, &req1);
			MPI_Wait(&req1, MPI_STATUSES_IGNORE);
			MPI_Irecv(subnumar2, cat, MPI_INT, 0, 30, MPI_COMM_WORLD, &req2);
			MPI_Wait(&req2, MPI_STATUSES_IGNORE);
			...
			if (rank != 1) {
				MPI_Irecv(&received_carry, 1, MPI_INT, rank - 1, 50, MPI_COMM_WORLD, &req3);
				MPI_Wait(&req3, &status1);
			}
			...
			if (rank != nr_procese - 1)
				MPI_Isend(&carry, 1, MPI_INT, rank + 1, 50, MPI_COMM_WORLD, &req3);


			MPI_Request req4;
			// trimite suma partiala la procesul 0
			MPI_Isend(suma_partiala, cat, MPI_INT, 0, 40, MPI_COMM_WORLD, &req4);
		}
	}
