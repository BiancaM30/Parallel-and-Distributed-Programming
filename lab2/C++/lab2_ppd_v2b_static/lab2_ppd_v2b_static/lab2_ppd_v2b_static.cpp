...
double secvential(int N, int M, int F[MAX][MAX], int n, int m, double kernel[MAXK][MAXK]) {
...
int transformPixel(int N, int M, int n, int m, int line, int column, int F[MAX][MAX], double kernel[MAXK][MAXK]) {
	int s = 0;
	for (int i = 0; i < n; i++)
		for (int j = 0; j < m; j++) {
...
void workerLine(int N, int M, int F[MAX][MAX], int n, int m, double kernel[MAXK][MAXK], MyBarrier* barrier, int start, int end) {
	int upperBorder[MAXK][MAXK], lowerBorder[MAXK][MAXK];
...
void workerColumn(int N, int M, int F[MAX][MAX], int n, int m, double kernel[MAXK][MAXK], MyBarrier* barrier, int start, int end) {
	int upperBorder[MAXK][MAXK], lowerBorder[MAXK][MAXK];
	queue<TemporaryCopy> queue;

	for (int i = 0; i < N; i++)
		for (int j = start; j < start + n / 2; j++)
			upperBorder[i][j - start] = transformPixel(N, M, n, m, i, j, F, kernel);
	...
	for (int i = 0; i < N; i++)
		for (int j = end - n / 2; j < end; j++)
			lowerBorder[i][end - 1 - j] = transformPixel(N, M, n, m, i, j, F, kernel);
	...
	for (int i = 0; i < N; i++)
		for (int j = start; j < start + n / 2; j++)
			F[i][j] = upperBorder[i][j - start];

	for (int i = 0; i < N; i++)
		for (int j = end - n / 2; j < end; j++)
			F[i][j] = lowerBorder[i][end - 1 - j];
}

double paralel(int N, int M, int F[MAX][MAX], int n, int m, double kernel[MAXK][MAXK], int p) {
...
void printMatrix(int f[MAX][MAX], int N, int M) {
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < M; j++) {
			cout << f[i][j] << " ";
		}
...
bool verifyIdenticalMatrix(int F1[MAX][MAX], int F2[MAX][MAX], int N, int M) {
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < M; j++) {
			if (F1[i][j] != F2[i][j])
				return false;
...
int main(int argc, char* argv[]) {
	ifstream in("C:/Facultate/Cursuri facultate sem 5/PPD/Labs/Teme/lab2-ok/C++/lab2_ppd_v2b_static/lab2_ppd_v2b_static/date.txt");
	int N, M, n, m, p;
	in >> N >> M >> n >> m;
    p = 4;
	//p = atoi(argv[1]);
